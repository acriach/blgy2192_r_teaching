...
To use it the ggplot2 package needs to be installed. Hint: you can check useing the PAckages tab, bottom right window. Then load it using the function `library()` if you haven't already loaded it this R session.
```{r, eval = TRUE}
library(ggplot2)
```
...
In order to save this plot (which has trendlines and different symbols for each species), we first assign it to a variable, we could call it `penguin_plot`:
